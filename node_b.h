...
class Node {
 ...
 public:
  ...
  void addTblEntry(string dstip, string nxthopip, string srcip, int cost) {
    RoutingEntry entry;
    entry.dstip = dstip;
    entry.nexthop = nxthopip;
    entry.ip_interface = srcip;
    entry.cost = cost;
...
class RoutingNode: public Node {
 public:
 void sendMsg(vector<RoutingNode*> nd, int noOfEdges){
    ...
    for (int i = 0; i < interfaces.size(); i++){
      struct edge_info e;
      e.from = this->getName();
      e.fromIp = interfaces[i].first.getip();
      e.to = interfaces[i].second->getName();
      e.toIp = interfaces[i].first.getConnectedIp();
      e.cost = interfaces[i].first.getcost();
      edges.push_back(e);
    }

    for (int i = 0; i < nd.size(); ++i) {//sending to all nodes information about its neighbors
      nd[i]->recvMsg(nd, nd[i], edges, noOfEdges);
    }
  }
  void recvMsg(vector<RoutingNode*> nd, RoutingNode* n, vector<edge_info> edges, int noOfEdges);
};
