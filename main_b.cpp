...
void routingAlgo(vector<RoutingNode*> distanceVectorNodes, int noOfEdges);
    ...
    */
	...
	if(distanceVectorNodes[i]->getName() == name) {
	  ...
	  for(int j = 0 ; j < distanceVectorNodes.size(); j++) {
		...
		//   distanceVectorNodes[i]->add_info(name, name, 0);
		//   distanceVectorNodes[i]->setIp(myeth);
	      break;
	    }
	  }
	}
	...
	// cout<<"no of egdes"<<edges<<endl;
    /* The logic of the routing algorithm should go here */
    routingAlgo(distanceVectorNodes, edges);
